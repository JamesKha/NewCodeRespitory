...
public class Tictac {
    ...
    public static void main(String[] args) throws IOException
	{
		...
		while(true)
		{
		System.out.println("Hello from Jimmy Kha! Welcome to the TicTacToe program \nIf you want to play, Enter 'y', else Enter 'n'");
		char a = (char)br.read();
		br.readLine();
		...
		else
		{
			...
			if(b=='X')       //If the user decides to go first
			{
				...
				while(d<5)
				{
				   ...
				   c=c-1;
				   checkX++;
				   if(board[c]!=0)     //Condition to break the loop in case the position of the board is already occupied
				   {
				   ...
				   if(po!=9)
				   {
					   ...
					   else
					   {
						   System.out.println("\n");
						   obj.printboard(board);
						   System.out.println("\nYay! Looks like you win, great job\n\n");
						   break;
					   }
				   }
	...
	public void printboard(char game[])
	{
		for (int index = 0; index < 3; index++) {
			System.out.println("  "+'|'+"   "+'|');
			System.out.println(" "+game[(index*3)]+'|'+" "+game[(index*3)+1]+" "+'|'+" "+game[(index*3)+1]);
			System.out.println("  "+'|'+"   "+'|');
			System.out.println("---------");
		}
	}
